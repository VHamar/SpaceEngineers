...
private IMyTextSurface programBlockScreen;
...
public void Main(string argument, UpdateType updateSource)
{
    ...
    var powerState = GetPumpPowerState(iceAmount);

    foreach (var pump in icePumps)
    {
        pump.ApplyAction("OnOff_" + powerState);
    }
...
private string GetPumpPowerState(int iceAmount)
{
    if (iceAmount > maxIceAmount)
    {
        return "Off";
    }
    else
...
{
    var output = "---=== Usstan's Ice Controller ===---\n" +
        "\n" +
        "Ice in storage: " + iceAmount + "/" + maxIceAmount + "\n" +
        "\n" +
        groupName + ":\n";
